...
//Rare exceptions to indicate a bug in the _hellandheaven code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_hellandheaven = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_hellandheaven]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.hellandheaven.module"
