...
NSString * HellAndHeaven$ModuleRequireFormat = @"(function(exports){"
...
@implementation HellAndHeavenObject
...
{
	// create HellAndHeaven global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "HellAndHeaven" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_hellandheaven = [[HellAndHeavenObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_hellandheaven];
	
	NSString *_hellandheavenNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _hellandheavenNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_hellandheaven valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:HellAndHeaven$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.hellandheaven.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.hellandheaven.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
